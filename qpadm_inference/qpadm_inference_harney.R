...
## input_folder <- "/media/storage/stef_sim/inference_estimation/harney_model_mut_1.25e-8_rec_1.25e-8_seq_249e+6/eig"
...
## print(sprintf("Input folder %s exists: %d", input_folder, as.numeric(dir.exists(input_folder))))
## print(sprintf("Output folder %s exists: %d", output_folder, as.numeric(dir.exists(output_folder))))
## print(sprintf("Plotting functions %s exists: %d", plot_functions, as.numeric(file.exists(plot_functions))))
...
target <- "pop_14"
## all_ancestors <- paste("pop_", c(0:12), sep = '')
all_ancestors <- paste("pop_", c(0,3,5,6,7,8,9,10,12), sep = '')
...
for (i in 1:nrow(all_models)) {
  right_pops <- c(outpops, setdiff(all_ancestors, c(outpops, all_models[i,], target)))
...
# ** Run F2 and qpAdm
  ...
  f2_blocks_for_single_model <- f2_from_geno(
    ...
    pops = metadata_info$Population,
    inds = metadata_info$Ind_ID,
...
# ** Summarize the data
  ...
  for (i in 1:length(all_qpadms)) {
    ...
    pop_weights[[i]] <- unname(all_qpadms[[i]]$popdrop[1, c(7, 8)])
